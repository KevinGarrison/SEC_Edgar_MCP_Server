...
class Utils:
    ...
    def get_latest_filings_index(
        self,
        filings:dict=None
        )->dict:
        """
        This function looks up the latest indices for sec filing docs 
        """
    ...
    def create_base_df_for_sec_company_data(
        self,
        mapping_latest_docs:dict=None,
        filings:dict=None,
        cik:str=None
        )->pd.DataFrame:
        """
        Creates a dataframe with the ids, form and docs content found for a company
        """
    ...
    async def _fetch_selected_company_filings(
        self,
        ctx:Context,
        cik:str|int,
        accession:str,
        ...
        """
        Helper function to fetch the sec filings
        """
        try:
            headers = {'User-Agent': user_agent}
    ...
    async def fetch_all_filings(
        self,
        ctx:Context,
        sec__base_df:pd.DataFrame,
        user_agent:str
        ) -> pd.Series:
        """
        Fetches the sec filings
        """
        all_filings = []
        for _, row in sec__base_df.iterrows():
            filings_dict = await self._fetch_selected_company_filings(ctx=ctx, cik=str(int(row['cik'])), accession=row['accession_number'], filename=row['docs'], user_agent=user_agent)
